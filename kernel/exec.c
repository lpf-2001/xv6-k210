...
loadseg(pagetable_t pagetable, uint64 va, struct dirent *ep, uint offset, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if(sz - i < PGSIZE)
      ...
      n = PGSIZE;
    if(ep->e_func->eread_func(ep, 0, (uint64)pa, offset+i, n) != n)
      return -1;
  }
...
int exec(char *path, char **argv)
{
  ...
  // Check ELF header
  if(ep->e_func->eread_func(ep, 0, (uint64) &elf, 0, sizeof(elf)) != sizeof(elf))
    goto bad;
  ...
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    if(ep->e_func->eread_func(ep, 0, (uint64)&ph, off, sizeof(ph)) != sizeof(ph))
      goto bad;
